...
const sendEmailBrevo = async (orderData: OrderData): Promise<boolean> => {
  const emailHtml = `
    <!DOCTYPE html>
    <html>
    <head>
  ...
  `;
  const msg = {
    sender: { name: "My Shop", email: process.env.EMAIL_USER },
  ...
  try {
    ...
    const response = await fetch("https://api.brevo.com/v3/smtp/email", {
      ...
      headers: {
        ...
        "api-key": apiKey, // Use the validated variable
