...
const Shipping = () => {
  const { cart, total, amount, cartShipping, shipping } = useCart();
  console.log("shipping cart:", cart);
  ...
  useEffect(() => {
    // Initialize form with existing shipping data if available
    if (shipping) {
      setFormData((prev) => ({
        ...prev,
        ...shipping,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Phone validation
    const fullPhone = formatPhone(formData.phoneCode, formData.phone);
    if (!isValidPhone(fullPhone)) {
      setPhoneError("Please enter a valid international phone number");
      return;
    }

    // Save shipping data and navigate to payment
    cartShipping({
      ...formData,
      fullPhone,
    });
  ...
  const handleChange = (
    ...
    // For phone input, validate it contains only digits
    if (name === "phone" && value && !containsOnlyDigits(value)) {
      setPhoneError("Phone number can only contain digits");
      return; // Don't update the state if invalid
    }

    const updates = { ...formData, [name]: value };

    // For phone inputs, update both the field and validate
    if (name === "phone" || name === "phoneCode") {
      ...
      if (phoneCode && phone) {
        const fullPhone = formatPhone(phoneCode, phone);
        updates.fullPhone = fullPhone;
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-blue-50 to-indigo-100">
      ...
      <section className="relative h-screen flex items-center justify-center overflow-hidden bg-black">
        ...
        {/* Floating Icons */}
        <motion.div
          className="absolute top-16 left-16 text-2xl opacity-70"
          animate={floatingAnimation}
        >
          🚚
        </motion.div>
        <motion.div
          className="absolute top-24 right-24 text-xl opacity-60"
          animate={delayedFloatingAnimation(1)}
        >
          📦
        </motion.div>
        <motion.div
          className="absolute bottom-20 left-1/3 text-lg opacity-50"
        ...
        <div className="relative z-10 text-center text-white max-w-4xl mx-auto px-4 w-full">
          ...
          >
            ...
            swiftly ship your order to ensure maximum freshness.
          </motion.p>
        </div>
      </section>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          >
            ...
            >
              ...
              >
                Enter your delivery details to get your coffee journey started
              </motion.p>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                >
                  <button
                    type="submit"
                    className="w-full bg-linear-to-r from-blue-600 to-indigo-600 text-white font-bold py-4 px-6 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 text-lg"
                  >
                    Continue to Payment
                  </button>
                </motion.div>
          ...
          <div className="space-y-6">
            ...
            <motion.div
              ...
              className="bg-linear-to-br from-blue-500 to-indigo-600 text-white rounded-2xl shadow-lg p-6"
            >
              <h3 className="text-xl font-bold mb-4">🚀 Shipping Benefits</h3>
